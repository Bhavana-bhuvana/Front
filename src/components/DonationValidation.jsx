...
//     otherwise: yup.string().nullable(), // one-time → optional
//   }),

//  // Bank Name
// bankName: yup.string().when(["frequency", "paymentMode"], {
//   is: (frequency, paymentMode) =>
//     frequency === "monthly" && paymentMode === "E-Mandate",
...
export const DonationValidation = yup.object().shape({
  ...
  .when("idType", (idType, schema) => {
    switch (idType) {
      case "PAN Card":
        return schema.matches(/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/, "Invalid PAN format");
      case "Aadhar":
        return schema.matches(/^\d{12}$/, "Invalid Aadhar number");
      case "Driving license":
        return schema.matches(/^[A-Z]{2}[0-9]{2}\d{11}$/, "Invalid Driving License format");
      case "VoterID":
        return schema.matches(/^[A-Z]{3}[0-9]{7}$/, "Invalid Voter ID format");
      default:
        return schema;
    }
  }),
